...
def count_elements(pred_label, class_num):
    # 创建一个12维的全零数组
    counts = np.zeros(class_num)
    
    # 遍历pred_label，统计每个元素出现的次数
...
class ImageList(Dataset):
    ...
    def count_samples_per_class(self):
        ...
        for class_id, indices in class_dict.items():
            samples_per_class.append(len(indices))
    ...
    def _make_class_dict(self):
        ...
        for i, lb in enumerate(self.label.tolist()):
            ...
            if not cat_id in class_dict:
                class_dict[cat_id] = []
            class_dict[cat_id].append(i)
        return class_dict                       # 里面存的是每个类别的样本的index，这里的index指的是在传入的数据集中的index
