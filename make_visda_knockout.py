"""
Create VisDA-C (Knock out) based on VisDA-C dataset to simulate more situations of label shift.
Target domain: Adapt uniform distribution by knocking out 50% of data points from 3 randomly picked classes.
...
knockout = random.sample(cls_list, knockout_num)
prob_list = np.array([1.0] * cls_num)
prob_list[knockout] = 1.0 - knockout_prob
prob_list = prob_list.tolist()

train_path = './data/VISDA-C/image_list_train.txt'
val_path = './data/VISDA-C/image_list_val.txt'
save_folder = './data/VISDA-Knockout'
